...
import { X, Check, Layers, Palette, Hash, BookOpen } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 overflow-hidden">
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] bg-white rounded-[48px] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] flex flex-col md:flex-row overflow-hidden animate-in zoom-in fade-in duration-500">
        ...
        <div className="w-full md:w-64 bg-slate-50 border-r border-slate-100 p-8 flex-shrink-0 flex flex-col items-center justify-center gap-6 hidden md:flex">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Notebook Preview</p>
        </div>
          ...
          <div className="p-8 md:p-12 space-y-8">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500 font-medium">Create a manual notebook to start annotating.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <Hash size={12} /> Notebook Name
                </label>
                <input autoFocus type="text" placeholder="e.g. Physics Theory 101" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-[22px] px-6 py-4 text-lg md:text-xl font-bold outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all placeholder:text-slate-300" />
              </div>

              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <Palette size={12} /> Cover Design
                </label>
                <div className="flex flex-wrap items-center gap-3">
                  {COLORS.map(color => (
                    <button key={color.value} type="button" onClick={() => setSelectedColor(color.value)} className={`relative w-8 h-8 md:w-10 md:h-10 rounded-xl transition-all ${selectedColor === color.value ? 'scale-125 shadow-xl ring-4 ring-indigo-500/20' : 'hover:scale-110'}`} style={{ backgroundColor: color.value }}>
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <Layers size={12} /> Paper Style
                </label>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {TEMPLATES.map(template => (
                    <button key={template.id} type="button" onClick={() => setSelectedTemplate(template.id as any)} className={`flex items-center sm:flex-col gap-4 p-4 rounded-[28px] border-2 transition-all text-left sm:text-center ${selectedTemplate === template.id ? 'border-indigo-600 bg-indigo-50/50' : 'border-slate-100 hover:border-slate-200 bg-slate-50'}`}>
                      <div className="w-12 sm:w-full aspect-square bg-white rounded-xl shadow-sm p-2 flex-shrink-0">{template.icon}</div>
                      <div><span className={`block text-[10px] font-black uppercase tracking-widest ${selectedTemplate === template.id ? 'text-indigo-600' : 'text-slate-900'}`}>{template.name}</span></div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="pt-4">
                <button type="submit" className="w-full py-4 md:py-5 bg-slate-900 hover:bg-black text-white rounded-[26px] font-black text-lg md:text-xl shadow-2xl shadow-slate-200 transition-all active:scale-95 disabled:opacity-50 group" disabled={!title.trim()}>Create Workspace</button>
              </div>
            </form>
          </div>
        </div>
